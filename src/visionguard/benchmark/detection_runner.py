...
def run_detection_drift_experiment(
    ...
    if not frames: raise ValueError("no frames were evaluated")
    ...
    for severity in severities:
        ...
        records.append(DetectionDriftRecord(manifest.video_id, condition, float(severity), len(frames), float(drift_score), baseline_f1, drifted_f1, drifted_f1-baseline_f1, baseline_mean_confidence, float(np.mean(drifted_confidences)) if drifted_confidences else 0.0))
    return records
...
def image_statistics(frame_bgr: np.ndarray) -> np.ndarray:
    ...
    if image.ndim != 3 or image.shape[2] != 3: raise ValueError("frame must have shape (height, width, 3)")
